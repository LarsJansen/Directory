...
$sourceDir = resolve_path($options['source'] ?? 'storage/textfiles/programming');
$indexFile = resolve_path($options['index'] ?? 'storage/textfiles/programming/index.html');
$outputSql = resolve_path($options['output'] ?? 'storage/exports/textfiles_programming_import.sql');
$outputReport = preg_replace('/\.sql$/i', '_report.txt', $outputSql) ?: ($outputSql . '_report.txt');
$categoryPath = trim((string) ($options['category'] ?? 'text-archives/programming'), '/');
...
$tempSitesPath = tempnam(sys_get_temp_dir(), 'prog_sites_');
...
report_line($reportHandle, 'Textfiles.com Programming Import Report');
...
function process_entry(
    ...
    $shortDescription = 'Historical computing-era document: ' . $bodyDescription;
    $originalUrl = 'http://textfiles.com/programming/' . str_replace('%2F', '/', rawurlencode(str_replace('\\', '/', $relativePath)));
    $sourceKey = 'textfiles:programming:' . str_replace('\\', '/', $relativePath);
            ...
            if ($autoCreateCategories) {
                fwrite(
                    ...
                    category_sql_block(
                        ...
                        $parent,
                        humanize_slug($part),
                        'Auto-created by recursive textfiles programming importer.'
                    )
                );
            }
        }
...
function write_sql_header($handle, string $categoryPath, bool $recursive, bool $autoCreateCategories, bool $dryRun): void
{
    ...
    fwrite($handle, "-- Textfiles.com Programming recursive import generated by import_textfiles_programming.php
");
    ...
    fwrite($handle, "VALUES ('textfiles.com', 'Programming Textfiles Import', 'Generated from local textfiles programming dump (recursive)', 'completed', 0, 0, NOW(), NOW());\n\n");
    fwrite($handle, "SET @batch_id = LAST_INSERT_ID();\n");
    fwrite($handle, "SET @created_categories = 0;\n");
    fwrite($handle, "SET @imported_sites = 0;\n\n");

    fwrite($handle, category_sql_block('text-archives', null, 'Text Archives', 'Auto-created by recursive textfiles programming importer.'));
    if ($categoryPath !== 'text-archives') {
        ...
        foreach ($segments as $segment) {
            ...
            fwrite($handle, category_sql_block($running, $parent, humanize_slug($segment), 'Auto-created by recursive textfiles programming importer.'));
            $parent = $running;
        }
    }
}
