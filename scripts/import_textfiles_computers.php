...
$sourceDir = resolve_path($options['source'] ?? 'storage/textfiles/computers');
$indexFile = resolve_path($options['index'] ?? 'storage/textfiles/computers/index.html');
$outputSql = resolve_path($options['output'] ?? 'storage/exports/textfiles_computers_import.sql');
$outputReport = preg_replace('/\.sql$/i', '_report.txt', $outputSql) ?: ($outputSql . '_report.txt');
$categoryPath = trim((string) ($options['category'] ?? 'text-archives/computers'), '/');
...
$tempSitesPath = tempnam(sys_get_temp_dir(), 'comp_sites_');
...
report_line($reportHandle, 'Textfiles.com Computers Import Report');
...
function process_entry(
    ...
    $shortDescription = 'Historical computer-era document: ' . $bodyDescription;
    $originalUrl = 'http://textfiles.com/computers/' . str_replace('%2F', '/', rawurlencode(str_replace('\\', '/', $relativePath)));
    $sourceKey = 'textfiles:computers:' . str_replace('\\', '/', $relativePath);
...
function write_sql_header($handle, string $categoryPath, bool $recursive, bool $autoCreateCategories, bool $dryRun): void
{
    ...
    fwrite($handle, "-- Textfiles.com Computers recursive import generated by import_textfiles_computers.php
");
