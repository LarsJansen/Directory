 ...
 * Textfile Description Enricher (v4)
 *
 * Safe, idempotent CLI tool for improving weak imported descriptions
 * on text archive entries only.
 *
 * Usage:
 *   php scripts/enrich_text_descriptions.php --limit=100
 *   php scripts/enrich_text_descriptions.php --limit=100 --dry-run
 *   php scripts/enrich_text_descriptions.php --limit=100 --offset=500
 *   php scripts/enrich_text_descriptions.php --limit=100 --dry-run --upgrade-titles=1
 */
...
echo "=== Enrich Text Descriptions (v4) ===\n";
echo "Rows loaded: {$limit}\n";
...
if (method_exists($db, 'fetchAll')) {
    ...
    fwrite(STDERR, "Unsupported DB wrapper: expected fetchAll() or query().\n");
    exit(1);
}
...
function line_is_noise(string $line): bool
{
    ...
    if (preg_match('/^[\-\=\*\#\_\/\\\\\[\]\(\)\:\;\+\~\.\%\<\>\|]{8,}$/', $line)) {
        return true;
    }

    if (preg_match('/(\[\%\]|\[\:\=\:\]|\/%{5,}|_{8,}|={8,}|-{8,}|\*{8,})/i', $line)) {
        return true;
    }

    if (preg_match('/\b(PKUNZIP|PKZIP|ZIP file|Copr\.|All Rights Reserved)\b/i', $line)) {
        return true;
    }

    if (preg_match('/\b(FILE|FILENAME|PATH|SUBJECT|FROM|TO|DATE)\s*:/i', $line)) {
        return true;
    }

    if (preg_match('/\bcontinued from previous\b/i', $line)) {
        return true;
    }
    ...
    if (preg_match('/^[A-Z0-9\W\s]{40,}$/', $line)) {
        return true;
    }

    if (preg_match('/\.(txt|zip|exe|com|arc|lzh|gif|jpg|jpeg|bmp)\b/i', $line)) {
        return true;
    }

    if (looks_like_contact_or_listing($line)) {
        return true;
    }

    if (line_symbol_density_too_high($line)) {
        return true;
    }
...
function normalise_sentence_case(string $text): string
{
    ...
    $replacements = [
        ...
        '/\brbbs\b/i' => 'RBBS',
        '/\bqwk\b/i' => 'QWK',
        '/\battention\b/i' => 'ATtention',
        '/\bat\b command\b/i' => 'AT command',
        '/\bansi\b/i' => 'ANSI',
        '/\bms-dos\b/i' => 'MS-DOS',
        '/\bibm\b/i' => 'IBM',
        '/\bcp\/m\b/i' => 'CP/M',
...
function candidate_is_trustworthy(string $text): bool
{
    ...
    if (preg_match('/\b(continued from previous|all rights reserved|zip file)\b/i', $text)) {
        return false;
    }
    ...
    if (preg_match('/^(and|but|so|then|because|or|also|anyways|yep|hi ho)\b/i', $text)) {
        return false;
    }

    if (preg_match('/^[A-Z0-9\W\s]{50,}$/', $text)) {
        return false;
    }
...
function score_candidate(string $text): int
{
    ...
    if (preg_match('/\b(is|was|were|had|have|offers|explains|describes|tells|covers|shows|running|using|written|published|compiled|provides)\b/i', $text)) {
        $score += 8;
    }

    if (preg_match('/\b(BBS|bulletin board|sysop|modem|shareware|message base|FidoNet|Wildcat|WWIV|RBBS|QWK|telecommunications|Hayes)\b/i', $text)) {
        $score += 8;
    }

    if (preg_match('/\b(guide|faq|history|story|warning|application|list|magazine|tutorial|commandments|ethics|manual)\b/i', $text)) {
        $score += 5;
    }
...
function find_best_paragraph(string $body): ?string
{
    ...
    foreach ($blocks as $block) {
        ...
        if (!candidate_is_trustworthy($candidate)) {
            continue;
        }
...
function normalize_excerpt(string $text): string
{
    ...
    $text = normalise_sentence_case($text);
...
function summarise_by_title_patterns(string $title, string $candidate): ?string
{
    ...
    if (preg_match('/\b(wildcat|rbbs|wwiv|remote access|vision x|firstclass)\b/i', $titleLower . ' ' . $candidateLower)) {
...
function build_curated_description(string $title, string $candidate): ?string
{
    ...
    $sentence = extract_first_sentence($candidate);
    if ($sentence === '') {
        return null;
    }

    $sentence = normalize_excerpt($sentence);

    if (preg_match('/^[a-z]/', $sentence)) {
        return null;
    }

    if (preg_match('/^(And|But|So|Then|Because|Or|Also|Anyways|Yep|Hi ho)\b/', $sentence)) {
        return null;
    }
    ...
    if (looks_like_contact_or_listing($sentence)) {
        return null;
    }

    if (preg_match('/\b(anyways|lemme|yep|hi ho|lol)\b/i', $sentence)) {
        return null;
    }

    if (mb_strlen($sentence) < 70 || mb_strlen($sentence) > 240) {
        return null;
    }
...
function title_is_weak(string $title): bool
{
    ...
    if (preg_match('/^[a-z0-9_\-]+\.(txt|doc|nfo)$/i', $title)) {
        return true;
    }

    if (preg_match('/^[A-Z0-9_\-]{6,}$/', $title)) {
        return true;
    }
...
function upgrade_title_if_safe(string $title, string $body, ?string $sourceKey = null): ?string
{
    ...
    if (!title_is_weak($original)) {
        return null;
    }

    if (preg_match('/\bfidonet\b/i', $body)) {
        return 'FidoNet Document';
    }

    if (preg_match('/\bfaq\b/i', $body)) {
        return 'BBS FAQ';
    }

    if (preg_match('/\bguide\b/i', $body)) {
        return 'BBS Guide';
    }

    if (preg_match('/\blog\b/i', $body)) {
        return 'BBS Log';
    }
...
function description_already_good(?string $description): bool
{
    ...
    if (preg_match('/^Historical .* document:/i', $description)) {
        return false;
    }
