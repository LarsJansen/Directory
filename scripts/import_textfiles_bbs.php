...
$tempSitesPath = tempnam(sys_get_temp_dir(), 'bbs_sites_');
...
report_line($reportHandle, 'Textfiles.com BBS Import Report');
...
function process_entry(
    ...
    $originalUrl = 'http://textfiles.com/bbs/' . str_replace('%2F', '/', rawurlencode(str_replace('\\', '/', $relativePath)));
    $sourceKey = 'textfiles:bbs:' . str_replace('\\', '/', $relativePath);
            ...
            if ($autoCreateCategories) {
                fwrite(
                    ...
                    category_sql_block(
                        ...
                        $parent,
                        humanize_slug($part),
                        'Auto-created by recursive textfiles BBS importer.'
                    )
                );
            }
        }
...
function write_sql_header($handle, string $categoryPath, bool $recursive, bool $autoCreateCategories, bool $dryRun): void
{
    ...
    fwrite($handle, "VALUES ('textfiles.com', 'BBS Textfiles Import', 'Generated from local textfiles BBS dump (recursive)', 'completed', 0, 0, NOW(), NOW());\n\n");
    fwrite($handle, "SET @batch_id = LAST_INSERT_ID();\n");
    fwrite($handle, "SET @created_categories = 0;\n");
    fwrite($handle, "SET @imported_sites = 0;\n\n");

    fwrite($handle, category_sql_block('text-archives', null, 'Text Archives', 'Auto-created by recursive textfiles BBS importer.'));
    if ($categoryPath !== 'text-archives') {
        ...
        foreach ($segments as $segment) {
            ...
            fwrite($handle, category_sql_block($running, $parent, humanize_slug($segment), 'Auto-created by recursive textfiles BBS importer.'));
            $parent = $running;
        }
    }
}
