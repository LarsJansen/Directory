...
$sourceDir = resolve_path($options['source'] ?? 'storage/textfiles/magazines');
$indexFile = resolve_path($options['index'] ?? 'storage/textfiles/magazines/index.html');
$outputSql = resolve_path($options['output'] ?? 'storage/exports/textfiles_magazines_import.sql');
$outputReport = preg_replace('/\.sql$/i', '_report.txt', $outputSql) ?: ($outputSql . '_report.txt');
$categoryPath = trim((string) ($options['category'] ?? 'text-archives/magazines'), '/');
...
report_line($reportHandle, 'Textfiles.com Magazines Import Report');
...
function process_entry(
    ...
    $originalUrl = 'http://textfiles.com/magazines/' . str_replace('%2F', '/', rawurlencode(str_replace('\\', '/', $relativePath)));
    $sourceKey = 'textfiles:magazines:' . str_replace('\\', '/', $relativePath);
            ...
            if ($autoCreateCategories) {
                fwrite(
                    ...
                    category_sql_block(
                        ...
                        $parent,
                        humanize_slug($part),
                        'Auto-created by recursive textfiles magazines importer.'
                    )
                );
            }
        }
...
function write_sql_header($handle, string $categoryPath, bool $recursive, bool $autoCreateCategories, bool $dryRun): void
{
    ...
    fwrite($handle, "-- Textfiles.com Magazines recursive import generated by import_textfiles_magazines.php
");
    ...
    fwrite($handle, "VALUES ('textfiles.com', 'Magazines Textfiles Import', 'Generated from local textfiles magazines dump (recursive)', 'completed', 0, 0, NOW(), NOW());\n\n");
    fwrite($handle, "SET @batch_id = LAST_INSERT_ID();\n");
    fwrite($handle, "SET @created_categories = 0;\n");
    fwrite($handle, "SET @imported_sites = 0;\n\n");

    fwrite($handle, category_sql_block('text-archives', null, 'Text Archives', 'Auto-created by recursive textfiles magazines importer.'));
    if ($categoryPath !== 'text-archives') {
        ...
        foreach ($segments as $segment) {
            ...
            fwrite($handle, category_sql_block($running, $parent, humanize_slug($segment), 'Auto-created by recursive textfiles magazines importer.'));
            $parent = $running;
        }
    }
}
